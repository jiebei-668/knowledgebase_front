...
const formatAsyncRoutes = (menus: MenuItem[]) => {
  ...
  const routes = mapTree(menus, (item) => {
    ...
    // 针对特定路径添加props函数
    // 信息发布打回重新编辑的路由
    if (item.path === '/newsRelease/edit') {
      routeConfig.props = (route) => ({
        contentId: route.query.contentId ? Number(route.query.contentId) : null
      });
    }
    // 针对特定路径添加props函数
    // 信息发布打回重新编辑的路由
    if (item.path === '/activityRelease/edit') {
      routeConfig.props = (route) => ({
        contentId: route.query.contentId ? Number(route.query.contentId) : null
      });
    }
