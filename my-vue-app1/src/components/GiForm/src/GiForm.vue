...
<!--              <template #icon><icon-search /></template>-->
                <template #icon>
                    <!-- 自定义搜索图标部分 -->
                    <component :is="props.searchIcon || IconSearch" />
                </template>
...
const props = withDefaults(defineProps<Props>(), {
  ...
  hideFoldBtn: false,
  suffix: true,
    searchIcon: undefined // 默认不传递自定义图标
})
...
const getComponentBindProps = (item: ColumnItem) => {
  ...
  const ConfigMap = new Map<ColumnItem['type'], Partial<ColumnItem['props'] & { placeholder: string }>>([
    ...
    ['textarea', { allowClear: false, placeholder: `请填写${item.label}`, maxLength: 2000 }],
    ...
    ['date-picker', { allowClear: true, placeholder: `请选择${item.label}`}],
