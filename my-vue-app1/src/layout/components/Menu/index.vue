...
// 新增以下代码：组件挂载后打印
onMounted(() => {
    console.log('当前routeStore.routes:', routeStore.routes)
})
...
const menuMode = computed(() => {
  // 打印当前的 layout 值
  console.log('当前 appStore.layout 的值是:', appStore.layout);

  return ['left', 'mix'].includes(appStore.layout) ? 'vertical' : 'horizontal';
});
